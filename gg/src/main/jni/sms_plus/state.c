...
int system_save_state_mem(char *bufPtr)
{
    ...
    memcpy(bufPtr, Z80_Context, sizeof(Z80_Regs) - r);
    bufPtr+=sizeof(Z80_Regs) - r;

    #if defined(__LP64__)
    bufPtr -= 4;
    memcpy(bufPtr, &Z80_Context->extra_cycles, 4);
    bufPtr += 4;
    #endif
...
void system_load_state_mem(char *data)
{
    ...
    memcpy(Z80_Context, data, sizeof(Z80_Regs)) - r;
    data+=sizeof(Z80_Regs) - r;
    #if defined(__LP64__)
    data -= 4;
    memcpy(&Z80_Context->extra_cycles, data, 4);
    data += 4;
    #endif
