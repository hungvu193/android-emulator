...
class GbaEmulator: public Emulator {
private:
	bool zapperEnabled;
	unsigned int travel[40][256 * 256];
	bool stereo;
public:
	GbaEmulator() {
		LOGE("CONSTRUCTOR");
	...
	bool start(int gfxInit, int sfxInit, int generalInit) {
		...
		if (success) {
			if (generalInit >= 10000) {
				generalInit -= (generalInit / 10000) * 10000;
			}
	...
	bool emulate(int keys, int turbos, int numFramesToSkip) {
		...
		unsigned int* g = gfxBufs[workingGfx];
		for (int i = 0; i<256 * 240; i++) {
			g[i] = emuPalette[gfxBuf[i]];
		}
	...
	bool renderGL() {
		...
		glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 256, 240, GL_RGBA,
	...
	void appendToSfxBuffer(int32* data, int numStereoSamples) {
		...
		if (stereo) {
			...
			memcpy((short*) buf + (slen << 1), (short*) data, numBytes);
		}
		sfxLock.Unlock();
	}
	...
	bool renderHistory(JNIEnv *env, jobject bitmap, int pos, int w, int h) {
	    ...
	    this->render(env, bitmap, w, h, travel[idx]);
	    return true;
	}
	...
	bool doSaveHistoryState(int idx) {
		...
		memcpy((void*) (travel[idx]), (void*) gfxBufs[workingGfx], 256 * 256 * 4);
		return true;
	}
	...
	bool processCommand(const char *command) {
		if (strcmp("FDS_SWITCH_SIDE", command) == 0) {
			FCEU_FDSSelect();
			return true;
		}
		if (strcmp("FDS_INSERT_EJECT", command) == 0) {
			FCEU_FDSInsert();
			return true;
		}
		return true;
	...
	int getInt(const char *name) {
		if (strcmp("FDS_INITED", name) == 0) {
			return NOSTALGIA_IsFDSInited() == 0 ? 0 : 1;
		}
		if (strcmp("FDS_INSERTED_DISK", name) == 0) {
			return NOSTALGIA_GetInDisk();
		}
		if (strcmp("FDS_SELECTED_DISK", name) == 0) {
			return NOSTALGIA_GetSelDisk();
		}
		if (strcmp("FDS_TOTAL_SIDES", name) == 0) {
			return NOSTALGIA_GetTotalSides();
		}
		return -1;
	}
	...
	bool doLoadGame(const char *path, const char*batterySaveDir,
