...
bool Emulator::processCommand(const char *command) {
	return false;
}

int Emulator::getInt(const char *name) {
	return 0;
}
...
bool Emulator::loadGame(const char*path, const char*batteryPath,
...
void Emulator::initBuffers() {
	for (int i = 0; i < 3; i++) {
		...
		for (int j = 0; j < getGfxBufferSize(); j++) {
			gfxBufs[i][j] = 255;
		}
	}
	for (int i = 0; i < 2; i++) {
